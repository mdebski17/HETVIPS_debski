...
def mkplot_cf_redshifts(z_phot, z_spec, add_stats=False):
    '''
    ...
    z_spec : 1d array
        spec-z's ("truth")
    add_stats : False or float
        if float, fontsize of statistics
    '''
    ...
    if add_stats:
        ...
        ax1.text(np.max(lims)*.6, np.max(lims)*.2, s=txt, fontsize=add_stats)
